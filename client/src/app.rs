use crate::{
    requests::{users::{register, login}, nights::{create_night, delete_night, edit_night, get_all_nights}},
    types::AppState,
};
use api_types::{
    users::model:: UserJSONRequest,
    nights::{
	self,
	model::{Drunkness, Night, NightJSONRequest},
    }
};
use bson::doc;
use chrono::Datelike;
...
impl Default for ImOk {
	...
	for i in get_all_nights()
	    .unwrap()
	    .into_json::<nights::response::ResponseMany>()
	    .unwrap()
	    .data
	    .unwrap()
	    .iter()
